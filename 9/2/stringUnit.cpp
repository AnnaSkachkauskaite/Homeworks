...
void makeString (String *string)
{
	...
	while (buf != '\n')
	{
		addSymbol(string, buf);
		buf = getchar();
	}
}

int hash (String *string, int hashArr[maxWordLength])
{
	int result = 0;
	int length = strLength(string);
	char *str = new char[maxWordLength];
	strToChar(string, str);
	for (int i = length - 1; i >= 0; --i)
		result = (result + str[i] * hashArr[i]) % 123;
	return result;
}

void findOccures (String *string, String *sub)
{
	int hashArr[maxWordLength];
	hashArr[0] = 1;
	for (int i = 1; i < maxWordLength; ++i)
		hashArr[i] = hashArr[i - 1] * 7 % 123;
	int hsub = hash(sub, hashArr);
	for (int i = 0; i < strLength(string) - strLength(sub) + 1; ++i)
		if (hash(copy(string, i, (strLength(sub) + i - 1)), hashArr) == hsub)
			if (areEquial(copy(string, i, strLength(sub) + i - 1), sub))
				printf("%d  ", i);
}
